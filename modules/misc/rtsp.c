...
static int MediaAddES( vod_t *p_vod, vod_media_t *p_media, es_format_t *p_fmt )
{
    ...
    TAB_APPEND( (media_es_t **), p_media->i_es, p_media->es, p_es );			// sunqueen modify
...
static void* CommandThread( void *obj )
{
    ...
    for( ;; )
    {
        ...
        {
            TAB_APPEND( (vod_media_t **), p_sys->i_media, p_sys->media, cmd.p_media );			// sunqueen modify
            goto next;
        }
...
static rtsp_client_t *RtspClientNew( vod_media_t *p_media, char *psz_session )
{
    ...
    TAB_APPEND( (rtsp_client_t **), p_media->i_rtsp, p_media->rtsp, p_rtsp );			// sunqueen modify
...
//#warning Should use secure randomness here! (spoofing risk)			// sunqueen delete
...
//#warning Session ID should be securely random (spoofing risk)			// sunqueen delete
                    if( asprintf( &psz_new, "%lu", vlc_mrand48() ) < 0 )
                        return VLC_ENOMEM;
                    psz_session = psz_new;
                ...
                TAB_APPEND( (rtsp_client_es_t **), p_rtsp->i_es, p_rtsp->es, p_rtsp_es );			// sunqueen modify
