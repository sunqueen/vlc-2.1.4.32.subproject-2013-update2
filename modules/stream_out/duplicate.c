...
static int Open( vlc_object_t *p_this )
{
    ...
    for( p_cfg = p_stream->p_cfg; p_cfg != NULL; p_cfg = p_cfg->p_next )
    {
        if( !strncmp( p_cfg->psz_name, "dst", strlen( "dst" ) ) )
        {
            ...
            if( s )
            {
                TAB_APPEND( (sout_stream_t **), p_sys->i_nb_streams, p_sys->pp_streams, s );	// sunqueen modify
                TAB_APPEND( (sout_stream_t **), p_sys->i_nb_last_streams, p_sys->pp_last_streams,
                    p_last );			// sunqueen modify
                TAB_APPEND( (char **), p_sys->i_nb_select,  p_sys->ppsz_select, NULL );			// sunqueen modify
            }
        }
...
static sout_stream_id_t * Add( sout_stream_t *p_stream, es_format_t *p_fmt )
{
    ...
    for( i_stream = 0; i_stream < p_sys->i_nb_streams; i_stream++ )
    {
        ...
        TAB_APPEND( (void **), id->i_nb_ids, id->pp_ids, id_new );			// sunqueen modify
    }
