...
static sout_stream_id_t *Add( sout_stream_t *p_stream, es_format_t *p_fmt )
{
    ...
    vlc_mutex_lock( &p_sys->lock_es );
    TAB_APPEND( (sout_stream_id_t **), p_sys->i_es, p_sys->es, id );			// sunqueen modify
    vlc_mutex_unlock( &p_sys->lock_es );
...
//        int deadv[id->sinkc]; /* Dead sockets list */
        int *deadv = (int *)malloc(sizeof(int) * id->sinkc); 
		// sunqueen modify end
...
int rtp_add_sink( sout_stream_id_t *id, int fd, bool rtcp_mux, uint16_t *seq )
{
    ...
    vlc_mutex_lock( &id->lock_sink );
    INSERT_ELEM( (rtp_sink_t *), id->sinkv, id->sinkc, id->sinkc, sink );			// sunqueen modify
    if( seq != NULL )
...
void rtp_del_sink( sout_stream_id_t *id, int fd )
{
    ...
    for( int i = 0; i < id->sinkc; i++ )
    {
        ...
        {
            sink = id->sinkv[i];
            REMOVE_ELEM( (rtp_sink_t *), id->sinkv, id->sinkc, i );			// sunqueen modify
            break;
        }
    }
    vlc_mutex_unlock( &id->lock_sink );
