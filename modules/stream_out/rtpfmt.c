...
//                char hexa[2*p_fmt->i_extra +1];
				char *hexa = (char *)malloc(2*p_fmt->i_extra +1);
				// sunqueen modify end
                sprintf_hexa( hexa, (uint8_t *)p_fmt->p_extra, p_fmt->i_extra );			// sunqueen modify
                if( asprintf( &rtp_fmt->fmtp,
                              "profile-level-id=3; config=%s;", hexa ) == -1 )
                    rtp_fmt->fmtp = NULL;
				free(hexa);			// sunqueen add
            }
            break;
        }
        case VLC_CODEC_MP4A:
