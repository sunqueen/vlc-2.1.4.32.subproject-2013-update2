    ...
    for( int i = 0; i < rtsp->sessionc; i++ )
    {
        ...
        for( int j = 0; j < ses->trackc; j++ )
        {
            if( ses->trackv[j].id == id )
            {
                ...
                RtspTrackClose( tr );
                REMOVE_ELEM( (rtsp_strack_t *), ses->trackv, ses->trackc, j );			// sunqueen modify
            }
        }
    }
...
rtsp_session_t *RtspClientNew( rtsp_stream_t *rtsp )
{
    ...
    TAB_APPEND( (rtsp_session_t **), rtsp->sessionc, rtsp->sessionv, s );			// sunqueen modify
                     ...
                     uint32_t *ssrc, uint16_t *seq_init )
{
    ...
    else
    {
        ...
        INSERT_ELEM( (rtsp_strack_t *), session->trackv, session->trackc, session->trackc, track);			// sunqueen modify
    ...
    for (int i = 0; i < session->trackc; i++)
    {
        ...
        if (tr->sout_id == sout_id)
        {
            if (tr->setup_fd == -1)
            {
                 ...
                 * seq_init next time. */
                REMOVE_ELEM( (rtsp_strack_t *), session->trackv, session->trackc, i );			// sunqueen modify
                break;
            }
...
//                  + strlen( rtsp->psz_path )];
    char *control = (char *)malloc(sizeof("rtsp://[]:12345") + NI_MAXNUMERICHOST
                  + strlen( rtsp->psz_path ));			// sunqueen modify
...
//        char ip[NI_MAXNUMERICHOST], *ptr;
		char *ptr;
        char *ip = (char *)malloc(NI_MAXNUMERICHOST);			// sunqueen modify
        int port;
                 ...
                 tpt = transport_next( tpt ) )
            {
                ...
                else
                {
                    ...
                    if (tr == NULL)
                    {
                        ...
                        INSERT_ELEM( (rtsp_strack_t *), ses->trackv, ses->trackc, ses->trackc,			// sunqueen modify
                                     track );
                    }
...
//                                          - 1 ) + 1];
                char *info = (char *)malloc(ses->trackc * ( strlen( control ) + TRACK_PATH_SIZE
                          + sizeof("url=;seq=65535;rtptime=4294967295, ")
                                          - 1 ) + 1);			
				// sunqueen modify end
                size_t infolen = 0;
                    ...
                    for( int i = 0; i < ses->trackc; i++ )
                    {
                        if( ses->trackv[i].id == id )
                        {
                            ...
                            if (!(vod && ses->trackv[i].sout_id != NULL))
                                REMOVE_ELEM( (rtsp_strack_t *), ses->trackv, ses->trackc, i );			// sunqueen modify
                        }
                    }
                    RtspClientAlive(ses);
                }
