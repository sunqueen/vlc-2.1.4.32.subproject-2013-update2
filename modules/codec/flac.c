                      ...
                      unsigned current_frame, void *client_data )
{
    ...
    if( samples == 0 )
    {
        if( p_sys->i_headers == 1 )
        {
            ...
            memcpy((char *)p_enc->fmt_out.p_extra + 4, buffer, STREAMINFO_SIZE );			// sunqueen modify
            /* Fake this as the last metadata block */
            ((uint8_t*)p_enc->fmt_out.p_extra)[4] |= 0x80;
        }
        p_sys->i_headers++;
        return (FLAC__StreamEncoderWriteStatus)FLAC__STREAM_DECODER_WRITE_STATUS_CONTINUE;			// sunqueen modify
    }
...
static block_t *Encode( encoder_t *p_enc, block_t *p_aout_buf )
{
    ...
    {
        p_sys->p_buffer = (FLAC__int32 *)			// sunqueen modify
