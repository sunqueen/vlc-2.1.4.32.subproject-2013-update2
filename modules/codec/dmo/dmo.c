...
static int DecOpen( decoder_t *p_dec )
{
    ...
    if( p_dec->fmt_in.i_cat == AUDIO_ES )
    {
        ...
        dmo_input_type.subtype.Data1 = p_dec->fmt_in.i_original_fourcc ? p_dec->fmt_in.i_original_fourcc : p_dec->fmt_in.i_codec;			// sunqueen modify
    ...
    else
    {
        ...
        p_bih->biCompression = p_dec->fmt_in.i_original_fourcc ? p_dec->fmt_in.i_original_fourcc : p_dec->fmt_in.i_codec;			// sunqueen modify
                    ...
                    bool b_out )
{
    ...
    if( p_fmt->i_cat == AUDIO_ES )
    {
        ...
        dmo_partial_type.subtype.Data1 = p_fmt->i_original_fourcc ? p_fmt->i_original_fourcc : p_fmt->i_codec;			// sunqueen modify
...
static void *DecoderThread( void *data )
{
    ...
    for( ;; )
    {
        ...
        for( ;; )
        {
            ...
            if( !p_output )
                break;
            TAB_APPEND( (void **), p_sys->i_output, p_sys->pp_output, p_output );			// sunqueen modify
        }
        p_sys->pp_input = NULL;
        vlc_cond_signal( &p_sys->wait_output );
    }
