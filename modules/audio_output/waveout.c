...
static int WaveoutVolumeSet( audio_output_t *p_aout, float volume )
{
    ...
    else
    {
        ...
        uint32_t vol = cover_lroundf_wavout( volume /* * 0x7fff.fp0*/ );			// sunqueen modify
...
static int WaveoutMuteSet( audio_output_t * p_aout, bool mute )
{
    ...
    else
    {
        ...
        const HWAVEOUT hwo = sys->h_waveout;
        uint32_t vol = mute ? 0 : cover_lroundf_wavout( sys->f_volume /* * 0x7fff.fp0*/ );			// sunqueen modify
...
static void WaveoutPollVolume( void * aout )
{
    ...
    volume = volume_divide(volume); /* /= 0x7fff.fp0; */			// sunqueen modify
