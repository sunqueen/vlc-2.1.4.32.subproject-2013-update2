...
static int Open(vlc_object_t *object)
{
    ...
    /* Allocate structure */
    vd->sys = sys = (vout_display_sys_t *)calloc(1, sizeof(vout_display_sys_t));			// sunqueen modify
    if (!sys)
        return VLC_ENOMEM;
...
static int Direct3DCreate(vout_display_t *vd)
{
    ...
    OurDirect3DCreate9 =
        (LPDIRECT3D9 (__stdcall *)(UINT))GetProcAddress(sys->hd3d9_dll, "Direct3DCreate9");			// sunqueen modify
...
static const d3d_format_t d3d_formats[] = {
    ...
    { "YV12",       (D3DFORMAT)MAKEFOURCC('Y','V','1','2'),    VLC_CODEC_YV12,  0,0,0 },
    { "YV12",       (D3DFORMAT)MAKEFOURCC('Y','V','1','2'),    VLC_CODEC_I420,  0,0,0 },
    { "YV12",       (D3DFORMAT)MAKEFOURCC('Y','V','1','2'),    VLC_CODEC_J420,  0,0,0 },
	// sunqueen modify end
    ...
    { NULL, (D3DFORMAT)0, 0, 0,0,0}			// sunqueen modify
};
...
static int Direct3DLockSurface(picture_t *picture)
{
    ...
    CommonUpdatePicture(picture, NULL, (uint8_t *)d3drect.pBits, d3drect.Pitch);			// sunqueen modify
    return VLC_SUCCESS;
}
/**
...
static int Direct3DCreatePool(vout_display_t *vd, video_format_t *fmt)
{
    ...
    *fmt = vd->source;
    ...
    rsc->p_sys = (picture_sys_t *)malloc(sizeof(*rsc->p_sys));			// sunqueen modify
    if (!rsc->p_sys) {
                                     ...
                                     subpicture_t *subpicture)
{
    ...
    *count_ptr = count;
    *region    = (d3d_region_t *)calloc(count, sizeof(**region));			// sunqueen modify
    if (*region == NULL) {
        *count_ptr = 0;
        return;
    }
    ...
    for (subpicture_region_t *r = subpicture->p_region; r; r = r->p_next, i++) {
        ...
        if (SUCCEEDED(hr)) {
            uint8_t  *dst_data   = (uint8_t *)lock.pBits;			// sunqueen modify
