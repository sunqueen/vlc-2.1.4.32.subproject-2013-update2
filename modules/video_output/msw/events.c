...
static HCURSOR EmptyCursor( HINSTANCE instance )
{
    ...
    HCURSOR cursor = NULL;
    uint8_t *and = (uint8_t *)malloc(cw * ch);			// sunqueen modify
    uint8_t *xor = (uint8_t *)malloc(cw * ch);			// sunqueen modify
    if( and && xor )
    {
...
event_thread_t *EventThreadCreate( vout_display_t *vd)
{
    ...
    event_thread_t *p_event = (event_thread_t *)malloc( sizeof(*p_event) );			// sunqueen modify
    if( !p_event )
        return NULL;
