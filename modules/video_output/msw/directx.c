...
static BOOL WINAPI DirectXOpenDDrawCallback(GUID *guid, LPSTR desc,
                                            ...
                                            HMONITOR hmon)
{
    vout_display_t *vd = (vout_display_t *)context;			// sunqueen modify
    vout_display_sys_t *sys = vd->sys;
    ...
    if (hmon == sys->hmonitor) {
        ...
        sys->display_driver = (GUID *)malloc(sizeof(*guid));			// sunqueen modify
...
static int DirectXOpenDDraw(vout_display_t *vd)
{
    ...
    OurDirectDrawCreate =
        (HRESULT (__stdcall *)(GUID *,LPDIRECTDRAW *,IUnknown *))GetProcAddress(sys->hddraw_dll, "DirectDrawCreate");			// sunqueen modify
    ...
    OurDirectDrawEnumerateEx =
      (HRESULT (__stdcall *)(LPDDENUMCALLBACKEXA,LPVOID,DWORD))GetProcAddress(sys->hddraw_dll, DIRECTDRAWENUMERATEEX_NAME);			// sunqueen modify
    ...
    sys->ddobject = (LPDIRECTDRAW2)ptr;			// sunqueen modify
...
static int DirectXOpenDisplay(vout_display_t *vd)
{
    ...
    sys->display = (LPDIRECTDRAWSURFACE2)ptr;			// sunqueen modify
                                ...
                                int backbuffer_count)
{
    ...
    HRESULT hr = IDirectDrawSurface_QueryInterface(surface_v1,
                                                   IID_IDirectDrawSurface2,			// sunqueen modify
                                                   (LPVOID *)surface);
                                           ...
                                           DWORD fourcc)
{
    ...
    DWORD *list = (DWORD *)calloc(count, sizeof(*list));			// sunqueen modify
    if (!list)
        return VLC_ENOMEM;
                                        ...
                                        video_format_t *fmt)
{
    ...
    /* */
    picture_resource_t *rsc = &sys->resource;
    rsc->p_sys = (picture_sys_t *)calloc(1, sizeof(*rsc->p_sys));			// sunqueen modify
    if (!rsc->p_sys)
        return VLC_ENOMEM;
...
static int DirectXLock(picture_t *picture)
{
    ...
    CommonUpdatePicture(picture, NULL, (uint8_t *)ddsd.lpSurface, ddsd.lPitch);			// sunqueen modify
    return VLC_SUCCESS;
}
                               ...
                               char ***values, char ***descs)
{
    ...
    ctx.values = (char **)xmalloc(sizeof(char *));			// sunqueen modify
    ctx.descs = (char **)xmalloc(sizeof(char *));			// sunqueen modify
    ctx.values[0] = strdup("");
    ...
    if (hddraw_dll != NULL)
    {
        ...
        HRESULT (WINAPI *OurDirectDrawEnumerateEx)(LPDDENUMCALLBACKEXA,
                                                   LPVOID, DWORD) =
              (HRESULT (__stdcall *)(LPDDENUMCALLBACKEXA,LPVOID,DWORD))GetProcAddress(hddraw_dll, DIRECTDRAWENUMERATEEX_NAME);			// sunqueen modify
    ...
    *values = ctx.values;
    *descs = ctx.descs;
    return (int)ctx.count;			// sunqueen modify
}
