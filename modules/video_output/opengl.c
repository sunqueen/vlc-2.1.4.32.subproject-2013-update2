...
vout_display_opengl_t *vout_display_opengl_New(video_format_t *fmt,
                                               ...
                                               vlc_gl_t *gl)
{
    vout_display_opengl_t *vgl = (vout_display_opengl_t *)calloc(1, sizeof(*vgl));			// sunqueen modify
    if (!vgl)
        return NULL;
...
#   endif
    ...
    if (supports_shaders && (need_fs_yuv || need_fs_xyz|| need_fs_rgba)) {
        ...
        for (unsigned j = 0; j < 3; j++) {
            ...
            char *infolog = (char *)malloc(infoLength);			// sunqueen modify
            int charsWritten;
                                ...
                                picture_t *picture, subpicture_t *subpicture)
{
    ...
    if (subpicture) {
        ...
        vgl->region       = (gl_region_t *)calloc(count, sizeof(*vgl->region));			// sunqueen modify
