...
static int  ReadMeta( demux_t *p_demux, uint8_t **pp_streaminfo, int *pi_streaminfo )
{
    ...
    *pp_streaminfo = NULL;
    ...
    s->i_time_offset = 0;
    s->i_byte_offset = 0;
    TAB_APPEND( (seekpoint_t **), p_sys->i_seekpoint, p_sys->seekpoint, s );			// sunqueen modify
...
static void ParsePicture( demux_t *p_demux, const uint8_t *p_data, int i_data )
{
    ...
    TAB_APPEND( (input_attachment_t **), p_sys->i_attachments, p_sys->attachments, p_attachment );			// sunqueen modify
}
