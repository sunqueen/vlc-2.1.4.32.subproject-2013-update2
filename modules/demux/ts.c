 ...
 * $Id: 1b8389b63037a62dd2f9549d2fb87399845936be $
 *
...
static int UserPmt( demux_t *p_demux, const char *psz_fmt )
{
    ...
    prg->handle     = dvbpsi_AttachPMT(
        ...
        PMTCallBack, p_demux );
#endif
    TAB_APPEND( (ts_prg_psi_t **), pmt->psi->i_prg, pmt->psi->prg, prg );			// sunqueen modify
...
static void PIDInit( ts_pid_t *pid, bool b_psi, ts_psi_t *p_owner )
{
    ...
    if( b_psi )
    {
        ...
        if( p_owner )
        {
            ...
            TAB_APPEND( (ts_prg_psi_t **), pid->psi->i_prg, pid->psi->prg, prg );			// sunqueen modify
        }
    }
    else
    {
    ...
    else
    {
        for( unsigned i = 0; i < i_page; i++ )
        {
            ...
            else
            {
                ...
                TAB_APPEND( (ts_es_t **), pid->i_extra_es, pid->extra_es, p_es );			// sunqueen modify
            }
...
static void PMTCallBack( void *data, dvbpsi_pmt_t *p_pmt )
{
    ...
    for( int i = 0; i < 8192; i++ )
    {
        ...
        {
            TAB_APPEND( (ts_pid_t **), i_clean, pp_clean, pid );			// sunqueen modify
        }
    }
    if( prg->iod )
    {
...
static void PATCallBack( void *data, dvbpsi_pat_t *p_pat )
{
    ...
    if( p_sys->i_pmt > 0 )
    {
        ...
        for( int i = 0; i < p_sys->i_pmt; i++ )
        {
            ...
            TAB_APPEND( (ts_pid_t **), i_pmt_rm, pmt_rm, pmt );			// sunqueen modify
        }
