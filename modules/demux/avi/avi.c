 ...
 * $Id: 2a50ad74a3907fc9f6f5a780f1ac7ae415c67835 $
 *
...
static int Open( vlc_object_t * p_this )
{
    ...
    for( i = 0 ; i < i_track; i++ )
    {
        ...
        TAB_APPEND( (avi_track_t **), p_sys->i_track, p_sys->track, tk );			// sunqueen modify
        es_format_Clean( &fmt );
    }
...
//    avi_index_t p_idx_idx1[p_sys->i_track];
    ...
    free(p_idx_indx);			// sunqueen add
    free(p_idx_idx1);			// sunqueen add
}
                                 ...
                                 avi_chunk_STRING_t *p_strn )
{
    ...
    p_attachment = vlc_input_attachment_New( psz_name,
                                             ...
                                             p, i_payload );
    if( p_attachment )
        TAB_APPEND( (input_attachment_t **), p_sys->i_attachment, p_sys->attachment, p_attachment );			// sunqueen modify
    free( psz_name );
