...
//    AVDictionary *options[p_sys->ic->nb_streams ? p_sys->ic->nb_streams : 1];
    AVDictionary **options = (AVDictionary **)malloc(sizeof(AVDictionary **) * (p_sys->ic->nb_streams ? p_sys->ic->nb_streams : 1));
    options[0] = NULL;
...
#else
//# warning FIXME: implement palette transmission			// sunqueen delete
#endif
            psz_type = "video";
            ...
            if( cc->codec_type == AVMEDIA_TYPE_ATTACHMENT )
            {
                ...
                if( cc->codec_id == AV_CODEC_ID_TTF )
                {
                    ...
                    if( filename && filename->value )
                    {
                        ...
                        TAB_APPEND( (input_attachment_t **), p_sys->i_attachments, p_sys->attachments,			// sunqueen modify
                                p_attachment );
                    }
                }
        ...
        if( cc->codec_type != AVMEDIA_TYPE_DATA )
        {
            ...
            TAB_APPEND( (es_out_id_t **), p_sys->i_tk, p_sys->tk, es );			// sunqueen modify
        }
    }
    ...
    for( i = 0; i < p_sys->ic->nb_chapters; i++ )
    {
        ...
        TAB_APPEND( (seekpoint_t **), p_sys->p_title->i_seekpoint, p_sys->p_title->seekpoint, s );			// sunqueen modify
    }
