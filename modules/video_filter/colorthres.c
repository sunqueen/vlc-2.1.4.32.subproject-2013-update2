                          ...
                          uint32_t i_color )
{
    ...
    *refu = i_u - 0x80;
    *refv = i_v - 0x80;
    *reflength = sqrt((double)(*refu * *refu + *refv * *refv));			// sunqueen modify
}
...
{
    int length = sqrt((double)(u * u + v * v));			// sunqueen modify
                         ...
                         vlc_value_t oldval, vlc_value_t newval, void *p_data )
{
    ...
    if( !strcmp( psz_var, CFG_PREFIX "color" ) )
        atomic_store( &p_sys->i_color, (atomic_int)newval.i_int );			// sunqueen modify
    else if( !strcmp( psz_var, CFG_PREFIX "similaritythres" ) )
        atomic_store( &p_sys->i_simthres, (atomic_int)newval.i_int );			// sunqueen modify
    else /* CFG_PREFIX "saturationthres" */
        atomic_store( &p_sys->i_satthres, (atomic_int)newval.i_int );			// sunqueen modify
