                            ...
                            bool process_chroma )
{
    ...
    for( ; p_out < p_out_end ; p_out += 2*p_dst->p[i_plane].i_pitch )
    {
        ...
        for( ; x < w8; x += 8 )
        {
            movq_m2r( po[0], mm0 );			// sunqueen modify

            psrlq_r2r( mm1, mm0 );
            pand_r2r(  mm2, mm0 );

            movq_r2m( mm0, po[0] );			// sunqueen modify
			po++;			// sunqueen modify
        }
    ...
    if( process_chroma )
    {
             ...
             i_plane++ )
        {
            ...
            for( ; p_out < p_out_end ; p_out += 2*p_dst->p[i_plane].i_pitch )
            {
                ...
                static const mmx_t b128 = { /*.uq =*/ 0x8080808080808080ULL };			// sunqueen modify
                movq_m2r( b128, mm5 );
                ...
                for( ; x < w8; x += 8 )
                {
                    movq_m2r( po8[0], mm0 );			// sunqueen modify
                    ...
                    movq_r2m( mm1, po8[0] );			// sunqueen modify
					po8++;			// sunqueen modify
                }
