...
static int Create( vlc_object_t *p_this )
{
    ...
    /* Allocate structure */
    p_filter->p_sys = (filter_sys_t *)malloc( sizeof( *p_filter->p_sys ) );			// sunqueen modify
    ...
    p_filter->p_sys->p_old_data = (uint8_t *)calloc( p_filter->fmt_in.video.i_width *			// sunqueen modify
                           ...
                           void *p_data )
{
    ...
    if( !strcmp( psz_var, FILTER_PREFIX "window-size" ) )
        atomic_store( &p_sys->i_window_size, (atomic_int)newval.i_int );			// sunqueen modify
    else if( !strcmp( psz_var, FILTER_PREFIX "softening-size" ) )
        atomic_store( &p_sys->i_softening, (atomic_int)newval.i_int );			// sunqueen modify
