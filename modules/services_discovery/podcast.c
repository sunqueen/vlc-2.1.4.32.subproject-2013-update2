    ...
    for( ;; )
    {
        ...
        for( int i = 0; i < p_sd->p_sys->i_input; i++ )
        {
            ...
            if( p_input->b_eof || p_input->b_error )
            {
                ...
                p_sd->p_sys->pp_input[i] = NULL;
                REMOVE_ELEM( (input_thread_t **), p_sys->pp_input, p_sys->i_input, i );			// sunqueen modify
                i--;
            }
        }
...
static void ParseUrls( services_discovery_t *p_sd, char *psz_urls )
{
    ...
    for( ;; )
    {
        ...
        if( i == p_sys->i_urls )
        {
            INSERT_ELEM( (char **), ppsz_new_urls, i_new_urls, i_new_urls,			// sunqueen modify
                         strdup( psz_urls ) );
            ...
            INSERT_ELEM( (input_item_t **), pp_new_items, i_new_items, i_new_items, p_input );			// sunqueen modify
            services_discovery_AddItem( p_sd, p_input, NULL /* no cat */ );

            INSERT_ELEM( (input_thread_t **), p_sys->pp_input, p_sys->i_input, p_sys->i_input,			// sunqueen modify
        ...
        else
        {
            ...
            INSERT_ELEM( (input_item_t **), pp_new_items, i_new_items, i_new_items, p_sys->pp_items[i] );			// sunqueen modify
        }
        if( psz_tok )
            psz_urls = psz_tok+1;
        else
...
static void ParseRequest( services_discovery_t *p_sd )
{
    ...
    *psz_tok = '\0';
    ...
    if( !strcmp( psz_request, "ADD" ) )
    {
        ...
        if( i == p_sys->i_urls )
        {
            INSERT_ELEM( (char **), p_sys->ppsz_urls, p_sys->i_urls, p_sys->i_urls,			// sunqueen modify
              strdup( psz_request ) );
            ...
            INSERT_ELEM( (input_item_t **), p_sys->pp_items, p_sys->i_items, p_sys->i_items, p_input );			// sunqueen modify
            services_discovery_AddItem( p_sd, p_input, NULL /* no cat */ );
    ...
    else if ( !strcmp( psz_request, "RM" ) )
    {
        ...
        if( i != p_sys->i_urls )
        {
            services_discovery_RemoveItem( p_sd, p_sys->pp_items[i] );
            vlc_gc_decref( p_sys->pp_items[i] );
            REMOVE_ELEM( (char **), p_sys->ppsz_urls, p_sys->i_urls, i );			// sunqueen modify
            REMOVE_ELEM( (input_item_t **), p_sys->pp_items, p_sys->i_items, i );			// sunqueen modify
        }
        SaveUrls( p_sd );
    }
