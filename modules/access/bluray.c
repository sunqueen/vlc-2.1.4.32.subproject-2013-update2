...
static int blurayOpen( vlc_object_t *object )
{
    ...
    /* */
    p_demux->p_sys = p_sys = (demux_sys_t *)calloc(1, sizeof(*p_sys));			// sunqueen modify
    if (unlikely(!p_sys)) {
        return VLC_ENOMEM;
    }
...
static es_out_id_t *esOutAdd( es_out_t *p_out, const es_format_t *p_fmt )
{
    ...
    if ( p_fmt->i_id >= 0 ) {
        ...
        if ( idx == -1 ) {
            fmt_es_pair_t *p_pair = (fmt_es_pair_t *)malloc( sizeof(*p_pair) );			// sunqueen modify
...
static es_out_t *esOutNew( demux_t *p_demux )
{
    ...
    es_out_t    *p_out = (es_out_t *)malloc( sizeof(*p_out) );			// sunqueen modify
    ...
    p_out->p_sys = (es_out_sys_t *)malloc( sizeof(*p_out->p_sys) );			// sunqueen modify
...
static subpicture_region_t* subpicture_region_Clone(subpicture_region_t *p_region_src)
{
    ...
    if (p_region_src->p_style != NULL) {
        p_region_dst->p_style = (text_style_t *)malloc(sizeof(*p_region_dst->p_style));			// sunqueen modify
...
static void blurayInitOverlay(demux_t *p_demux, const BD_OVERLAY* const ov)
{
    ...
    p_sys->p_overlays[ov->plane] = (bluray_overlay_t *)calloc(1, sizeof(**p_sys->p_overlays));			// sunqueen modify
    if (unlikely(!p_sys->p_overlays[ov->plane]))
        return;

    subpicture_updater_sys_t *p_upd_sys = (subpicture_updater_sys_t *)malloc(sizeof(*p_upd_sys));			// sunqueen modify
...
static int blurayInitTitles(demux_t *p_demux )
{
    ...
    for (unsigned int i = 0; i < i_title; i++) {
        ...
        for ( unsigned int j = 0; j < title_info->chapter_count; j++) {
            ...
            TAB_APPEND( (seekpoint_t **), t->i_seekpoint, t->seekpoint, s );			// sunqueen modify
        }
        TAB_APPEND( (input_title_t **), p_sys->i_title, p_sys->pp_title, t );			// sunqueen modify
...
static int blurayControl(demux_t *p_demux, int query, va_list args)
{
    ...
    switch (query) {
        ...
        case DEMUX_GET_TITLE_INFO:
        {
            ...
            *ppp_title = (input_title_t **)calloc( p_sys->i_title, sizeof(input_title_t **) );			// sunqueen modify
            for( unsigned int i = 0; i < p_sys->i_title; i++ )
...
static void blurayHandleEvent( demux_t *p_demux, const BD_EVENT *e )
{
    ...
    switch (e->event)
    {
        ...
        case BD_EVENT_AUDIO_STREAM:
        {
            ...
            if ( idx >= 0 ) {
                es_out_id_t *p_es = (es_out_id_t *)vlc_array_item_at_index(&p_sys->es, idx);			// sunqueen modify
