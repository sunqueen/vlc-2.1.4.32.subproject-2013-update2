...
static bool ImportNextFile( access_t *p_access )
{
    ...
    ARRAY_APPEND( (uint64_t *), p_sys->file_sizes, st.st_size );			// sunqueen modify
...
static void ImportMeta( access_t *p_access )
{
    ...
    char *psz_display = (char *)malloc( i_len );			// sunqueen modify
...
static void ImportMarks( access_t *p_access )
{
    ...
    while( ReadLine( &line, &line_len, marksfile ) )
    {
        ...
        TAB_APPEND( (seekpoint_t **), p_marks->i_seekpoint, p_marks->seekpoint, sp );			// sunqueen modify
    }
    ...
    if( p_marks->i_seekpoint > 0 && p_marks->seekpoint[0]->i_byte_offset > 0 )
    {
        ...
        if( sp )
        {
            ...
            TAB_INSERT( (seekpoint_t **), p_marks->i_seekpoint, p_marks->seekpoint, sp, 0 );			// sunqueen modify
        }
    }
