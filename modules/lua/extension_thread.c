...
int Activate( extensions_manager_t *p_mgr, extension_t *p_ext )
{
    ...
    ARRAY_APPEND( (extension_t **), p_mgr->p_sys->activated_extensions, p_ext );			// sunqueen modify
...
static int RemoveActivated( extensions_manager_t *p_mgr, extension_t *p_ext )
{
    ...
    if( i_idx >= 0 )
    {
        ARRAY_REMOVE( (extension_t **), p_mgr->p_sys->activated_extensions, i_idx );			// sunqueen modify
    }
    else
    {
