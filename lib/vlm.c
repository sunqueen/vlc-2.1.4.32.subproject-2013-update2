 ...
 * $Id: aad066bc77f6ebaac8972e28005602bbff32aa6b $
 *
                              ...
                              int b_enabled, int b_loop )
{
    ...
    if( psz_input )
        TAB_APPEND( (char **), m.i_input, m.ppsz_input, strdup(psz_input) );			// sunqueen modify
    if( psz_output )
        m.psz_output = strdup( psz_output );
    for( n = 0; n < i_options; n++ )
        TAB_APPEND( (char **), m.i_option, m.ppsz_option, strdup(ppsz_options[n]) );			// sunqueen modify
...
#define VLM_CHANGE_CODE { while( p_media->i_input > 0 ) \
                          ...
                          TAB_APPEND( (char **), p_media->i_input, p_media->ppsz_input, \
                                      strdup(psz_input) ); }
...
{
	// sunqueen modify
#define VLM_CHANGE_CODE { TAB_APPEND( (char **), p_media->i_input, p_media->ppsz_input, \
                          strdup(psz_input) ); }
    VLM_CHANGE( "Unable to change %s input property", VLM_CHANGE_CODE );
#undef VLM_CHANGE_CODE
}
...
#define VLM_CHANGE_CODE { int n;        \
    ...
    while( p_media->i_input > 0 )       \
        free( p_media->ppsz_input[--p_media->i_input] );    \
    if( psz_input )                     \
        TAB_APPEND( (char **), p_media->i_input, p_media->ppsz_input, strdup(psz_input) ); \
    ...
    for( n = 0; n < i_options; n++ )    \
        TAB_APPEND( (char **), p_media->i_option, p_media->ppsz_option, \
