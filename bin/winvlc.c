...
# define PROCESS_DEP_ENABLE 1
        ...
        mySetDllDirectoryA = (BOOL (__stdcall *)(const char *))
                            GetProcAddress(h_Kernel32, "SetDllDirectoryA");			// sunqueen modify
...
//    char *argv[argc + 3];
    ...
    _setmode( fileno(stdin)/*STDIN_FILENO*/, _O_BINARY ); /* Needed for pipes */			// sunqueen modify
    ...
    if (vlc != NULL)
    {
        ...
        MessageBox (NULL, TEXT("VLC media player could not start.\nEither the command line options were invalid or no plugins were found.\n"),
                    ...
                    MB_OK|MB_ICONERROR);			// sunqueen modify
...
static void check_crashdump(void)
{
    ...
    int answer = MessageBox( NULL, L"Ooops: VLC media player just crashed.\nWould you like to send a bug report to the developers team?",			// sunqueen modify
        ...
        if(Hint)
        {
            ...
            if(ftp)
            {
                ...
                if( FtpPutFile( ftp, crashdump_path, remote_file,
                    ...
                    MessageBox( NULL, L"Report sent correctly. Thanks a lot for the help.", L"Report sent", MB_OK);			// sunqueen modify
                else
