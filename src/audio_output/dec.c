 ...
 * $Id: 57d292fe0a9123beb2c557fd98e19816c3980805 $
 *
...
static int aout_CheckReady (audio_output_t *aout)
{
    ...
    int restart = atomic_exchange (&owner->restart, (atomic_uchar)0);			// sunqueen modify
    if (unlikely(restart))
    {
        if (owner->mixer_format.i_format)
            aout_FiltersDelete (aout, owner->filters);
...
{
    aout_owner_t *owner = aout_owner (aout);
    atomic_fetch_or (&owner->restart, (atomic_uchar)mode);			// sunqueen modify
...
drop:
    ...
    block_Release (block);
lost:
    atomic_fetch_add(&owner->buffers_lost, (atomic_uint)1);			// sunqueen modify
    goto out;
}
