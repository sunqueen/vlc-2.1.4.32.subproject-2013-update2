 ...
 * $Id: 35e7a184c18b15d93c3a2b44dbe93ed32c692362 $
 *
                                 ...
                                 const vout_configuration_t *cfg)
{
    ...
    /* Allocate descriptor */
    vout_thread_t *vout = (vout_thread_t *)vlc_custom_create(object,			// sunqueen modify
...
static int VoutVideoFilterStaticAllocationSetup(filter_t *filter, void *data)
{
    ...
    filter->p_owner             = (filter_owner_sys_t *)data; /* vout */			// sunqueen modify
    return VLC_SUCCESS;
}
                                ...
                                bool is_locked)
{
    ...
    while (current) {
        ...
        if (name && *name) {
            vout_filter_t *e = (vout_filter_t *)xmalloc(sizeof(*e));			// sunqueen modify
            e->name = name;
            e->cfg  = cfg;
    ...
    for (int a = 0; a < 2; a++) {
        ...
        for (int i = 0; i < vlc_array_count(array); i++) {
            vout_filter_t *e = (vout_filter_t *)vlc_array_item_at_index(array, i);			// sunqueen modify
...
{
    vout_thread_t *vout = (vout_thread_t *)object;			// sunqueen modify
