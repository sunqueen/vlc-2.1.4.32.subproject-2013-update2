...
{
    filter_t *text = (filter_t *)vlc_custom_create(spu, sizeof(*text), "spu text");			// sunqueen modify
    if (!text)
        return NULL;

    text->p_owner = (filter_owner_sys_t *)xmalloc(sizeof(*text->p_owner));			// sunqueen modify
...
static filter_t *SpuRenderCreateAndLoadScale(vlc_object_t *object,
                                             ...
                                             bool require_resize)
{
    filter_t *scale = (filter_t *)vlc_custom_create(object, sizeof(*scale), "scale");			// sunqueen modify
    if (!scale)
        return NULL;
...
{
    spu_scale_t s = { /*.w =*/ w, /*.h =*/ h };			// sunqueen modify
    if (s.w <= 0)
        s.w = SCALE_UNIT;
    if (s.h <= 0)
        s.h = SCALE_UNIT;
...
{
    spu_area_t a = { /*.x =*/ x, /*.y =*/ y, /*.width =*/ w, /*.height =*/ h, /*.scale =*/ s };			// sunqueen modify
    return a;
}
                            ...
                            mtime_t render_date)
{
    ...
    *dst_area = spu_area_create(x_offset, y_offset,
    ...
    if (sys->scale && sys->scale->p_module &&
        ...
        if (region->p_private) {
            subpicture_region_private_t *_private = region->p_private;			// sunqueen modify
            bool is_changed = false;

            /* Check resize changes */
            if (dst_width  != _private->fmt.i_visible_width ||
                dst_height != _private->fmt.i_visible_height)			// sunqueen modify
                is_changed = true;
            ...
            if (convert_chroma && _private->fmt.i_chroma != chroma_list[0])			// sunqueen modify
                is_changed = true;

            if (is_changed) {
                subpicture_region_private_Delete(_private);			// sunqueen modify
                                          ...
                                          mtime_t render_osd_date)
{
    ...
    if (subtitle_region_count > sizeof(subtitle_area_buffer)/sizeof(*subtitle_area_buffer))
        subtitle_area = (spu_area_t *)calloc(subtitle_region_count, sizeof(*subtitle_area));			// sunqueen modify
...
{
    spu_t *spu = (spu_t *)data;			// sunqueen modify

    filter_owner_sys_t *sys = (filter_owner_sys_t *)malloc(sizeof(*sys));			// sunqueen modify
    if (!sys)
        return VLC_EGENERIC;
...
{
    spu_t *spu = (spu_t *)vlc_custom_create(object,			// sunqueen modify
