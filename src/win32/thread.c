...
{
    struct vlc_threadvar *var = (vlc_threadvar *)malloc (sizeof (*var));			// sunqueen modify
...
{
    struct vlc_thread *th = (vlc_thread *)malloc (sizeof (*th));			// sunqueen modify
    if (unlikely(th == NULL))
        return ENOMEM;
    th->entry = entry;
    th->data = data;
...
{
    struct vlc_thread *th = (struct vlc_thread *)self;				// sunqueen modify
...
static void SelectClockSource (vlc_object_t *obj)
{
...
#if (_WIN32_WINNT < 0x0601)
        HANDLE h = GetModuleHandle ((LPCSTR)_T("kernel32.dll"));
        if (unlikely(h == NULL))
            abort ();
        clk.interrupt.query = (BOOL (__cdecl *)(PULONGLONG))GetProcAddress ((HMODULE)h,			// sunqueen modify
    ...
    if (!strcmp (name, "tick"))
    {
        ...
        if (unlikely(h == NULL))
            abort ();
        clk.tick.get = (ULONGLONG (__cdecl *)(void))GetProcAddress ((HMODULE)h, "GetTickCount64");			// sunqueen modify
...
#define xstrdup(str) strdup(str)			// sunqueen modify

size_t EnumClockSource (vlc_object_t *obj, const char *var,
                        char ***vp, char ***np)
{
    const size_t max = 6;
    char **values = (char **)xmalloc (sizeof (*values) * max);			// sunqueen modify
    char **names = (char **)xmalloc (sizeof (*names) * max);			// sunqueen modify
    size_t n = 0;
...
{
    struct vlc_timer *timer = (vlc_timer *)val;			// sunqueen modify
...
{
    struct vlc_timer *timer = (vlc_timer *)malloc (sizeof (*timer));			// sunqueen modify
...
extern "C" vlc_rwlock_t config_lock;			// sunqueen modify
//BOOL WINAPI DllMain (HINSTANCE, DWORD, LPVOID);			// sunqueen delete
