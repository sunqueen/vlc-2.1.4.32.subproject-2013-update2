...
#   define ABOVE_NORMAL_PRIORITY_CLASS 0x00008000
       ...
       && var_InheritBool( p_this, "started-from-file" ) ) )
    {
		...
		if (!(hmutex = CreateMutex(0, TRUE, L"VLC ipc 2.1.4"/*VERSION */)))			// sunqueen modify
        {
        ...
        else
        {
             ...
             * 1st instance */
			if (!(ipcwindow = FindWindow(0, L"VLC ipc 2.1.4"/*VERSION*/)))			// sunqueen modify
            {
                msg_Err( p_this, "one instance mode DISABLED "
...
static unsigned __stdcall IPCHelperThread( void *data )
{
    ...
    ipcwindow =
        CreateWindow( L"STATIC",                     /* name of window class */
                  L"VLC ipc 2.1.4"/*VERSION*/,               /* window title bar text */			// sunqueen modify
                  0,                                         /* window style */
...
//#warning URI conversion must be done in calling process instead!			// sunqueen delete
...
void system_End(void)
{
    ...
    if (p_helper)
    {
        if( ( ipcwindow = FindWindow( 0, L"VLC ipc 2.1.4"/*VERSION*/ ) ) != 0 )			// sunqueen modify
        {
