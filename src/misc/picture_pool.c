...
{
    picture_pool_t *pool = (picture_pool_t *)calloc(1, sizeof(*pool));				// sunqueen modify
    if (!pool)
        return NULL;
    ...
    pool->picture = (picture_t **)calloc(pool->picture_count, sizeof(*pool->picture));			// sunqueen modify
    pool->picture_reserved = (bool *)calloc(pool->picture_count, sizeof(*pool->picture_reserved));			// sunqueen modify
...
picture_pool_t *picture_pool_NewExtended(const picture_pool_configuration_t *cfg)
{
    ...
    for (int i = 0; i < cfg->picture_count; i++) {
        ...
        picture_gc_sys_t *gc_sys = (picture_gc_sys_t *)malloc(sizeof(*gc_sys));			// sunqueen modify
        if (!gc_sys)
            abort();
...
//    picture_t *picture[picture_count];
	picture_t **picture = (picture_t **)malloc(sizeof(picture_t *) * picture_count);
	memset((char *)picture, '\0', sizeof(picture_t *) * picture_count);
	// sunqueen modify end
...
void picture_pool_Delete(picture_pool_t *pool)
{
    for (int i = 0; i < pool->picture_count; i++) {
        ...
        if (pool->master) {
            ...
            picture->gc.p_sys      = (picture_gc_sys_t *)gc_sys->destroy_sys;			// sunqueen modify
