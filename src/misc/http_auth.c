...
//    char psz_what[strlen(psz_param)+3];
	char *psz_what = (char *)malloc(strlen(psz_param)+3);
	memset(psz_what, '\0', strlen(psz_param)+3);
	// sunqueen modify end
...
//    char psz_what[strlen(psz_param)+2];
    char *psz_what = (char *)malloc(strlen(psz_param)+2);			// sunqueen modify
    sprintf( psz_what, "%s=", psz_param );
    psz_header = strstr( psz_header, psz_what );
    if ( psz_header )
    {
        const char *psz_end;
		char *psz_ret;			// sunqueen add
        psz_header += strlen( psz_what );
        psz_end = strchr( psz_header, ',' );
		...
		{
			// sunqueen modify start
            psz_ret = strdup( psz_header );
			free(psz_what);			// sunqueen add
			return psz_ret;
			// sunqueen modify end
		}
		// sunqueen modify start
        psz_ret = strndup( psz_header, psz_end - psz_header );
		free(psz_what);			// sunqueen add
		return psz_ret;
		// sunqueen modify end
    }
