...
#include <cover_search.h>			// sunqueen add
#include <limits.h>
#include <assert.h>
...
void *vlc_custom_create (vlc_object_t *parent, size_t length,
                         const char *type_name)			// sunqueen modify
{
    /* NOTE:
    ...
    vlc_object_internals_t *priv = (vlc_object_internals_t *)malloc (sizeof (*priv) + length);			// sunqueen modify
    ...
    obj->psz_object_type = type_name;			// sunqueen modify
    obj->psz_header = NULL;
    obj->b_force = false;
    ...
    if (likely(parent != NULL))
    {
        ...
        obj->p_parent = (vlc_object_t *)vlc_object_hold (parent);			// sunqueen modify
...
//            write (internals->pipes[1], &(uint64_t){ 1 }, sizeof (uint64_t));
			uint64_t data = 1;
            write (internals->pipes[1], (char *)&data, sizeof (uint64_t));
			// sunqueen modify end
        }
        vlc_restorecancel (canc);
    }
...
void ObjectKillChildrens( vlc_object_t *p_obj )
{
    ...
    if (atomic_exchange (&priv->alive, false))
    {
        ...
        if (fd != -1)
        {
			// sunqueen modify start
			uint64_t size = 1;
            write (fd, (char *)&size/*&(uint64_t){ 1 }*/, sizeof (uint64_t));
			// sunqueen modify end
...
#ifndef NDEBUG
    unsigned refs = atomic_fetch_add (&internals->refs, (atomic_uint)1);			// sunqueen modify
    assert (refs > 0); /* Avoid obvious freed object uses */
#else
    atomic_fetch_add (&internals->refs, (atomic_uint)1);			// sunqueen modify
#endif
    return p_this;
}
...
void vlc_object_release( vlc_object_t *p_this )
{
    ...
    /* Slow path */
    libvlc_lock (p_this->p_libvlc);
    refs = atomic_fetch_sub (&internals->refs, (atomic_uint)1);			// sunqueen modify
    assert (refs > 0);
...
vlc_list_t *vlc_list_children( vlc_object_t *obj )
{
    ...
    if (likely(l != NULL))
    {
        ...
        for (priv = vlc_internals (obj)->first; priv; priv = priv->next)
            l->p_values[i++].p_object = (vlc_object_t *)vlc_object_hold (vlc_externals (priv));				// sunqueen modify
    }
    libvlc_unlock (obj->p_libvlc);
    return l;
}
                        ...
                        vlc_value_t oldval, vlc_value_t newval, void *p_data )
{
    ...
    if( *newval.psz_string )
    {
        ...
        p_object = (vlc_object_t *)vlc_object_find_name( (vlc_object_t *)p_data, newval.psz_string );			// sunqueen modify
        if( !p_object )
            return VLC_ENOOBJ;
    }
    ...
    else if( *psz_cmd == 'v' )
    {
        ...
        if( vlc_internals( p_object )->var_root == NULL )
            ...
            cover_twalk( vlc_internals( p_object )->var_root, DumpVariable );			// sunqueen modify
...
static vlc_object_t *FindName (vlc_object_internals_t *priv, const char *name)
{
    if (priv->psz_name != NULL && !strcmp (priv->psz_name, name))
        return (vlc_object_t *)vlc_object_hold (vlc_externals (priv));			// sunqueen modify
...
{
    vlc_list_t * p_list = (vlc_list_t *)malloc( sizeof( vlc_list_t ) );			// sunqueen modify
    if( p_list == NULL )
        return NULL;
    ...
    p_list->p_values = (vlc_value_t *)malloc( i_count * sizeof( vlc_value_t ) );			// sunqueen modify
