...
static variable_t *Lookup( vlc_object_t *obj, const char *psz_name )
{
    ...
    pp_var = (variable_t **)cover_tfind( &psz_name, &priv->var_root, varcmp );				// sunqueen modify
...
int var_Create( vlc_object_t *p_this, const char *psz_name, int i_type )
{
     ...
     && var_Inherit( p_this, psz_name, i_type, &p_var->val ) == 0 )
    {
        if( i_type & VLC_VAR_HASCHOICE )
        {
            ...
            INSERT_ELEM( (vlc_value_t *), p_var->choices.p_values, p_var->choices.i_count,
                         0, p_var->val );				// sunqueen modify
            INSERT_ELEM( (vlc_value_t *), p_var->choices_text.p_values,
                         p_var->choices_text.i_count, 0, p_var->val );				// sunqueen modify
    ...
    pp_var = (variable_t **)cover_tsearch( p_var, &p_priv->var_root, varcmp );				// sunqueen modify
...
int var_Destroy( vlc_object_t *p_this, const char *psz_name )
{
    ...
    if( --p_var->i_usage == 0 )
        cover_tdelete( p_var, &p_priv->var_root, varcmp );			// sunqueen modify
    else
        p_var = NULL;
    vlc_mutex_unlock( &p_priv->var_lock );
                ...
                int i_action, vlc_value_t *p_val, vlc_value_t *p_val2 )
{
    ...
    switch( i_action )
    {
        ...
        case VLC_VAR_ADDCHOICE:
        {
            ...
            INSERT_ELEM( (vlc_value_t *), p_var->choices.p_values, p_var->choices.i_count,
                         i, *p_val );			// sunqueen modify
            INSERT_ELEM( (vlc_value_t *), p_var->choices_text.p_values,
                         p_var->choices_text.i_count, i, *p_val );			// sunqueen modify
        ...
        case VLC_VAR_DELCHOICE:
        {
            ...
            p_var->ops->pf_free( &p_var->choices.p_values[i] );
            free( p_var->choices_text.p_values[i].psz_string );
            REMOVE_ELEM( (vlc_value_t *), p_var->choices.p_values, p_var->choices.i_count, i );			// sunqueen modify
            REMOVE_ELEM( (vlc_value_t *), p_var->choices_text.p_values,
                         p_var->choices_text.i_count, i );			// sunqueen modify
                     ...
                     vlc_callback_t pf_callback, void *p_data )
{
    ...
    WaitUnused( p_this, p_var );
    INSERT_ELEM( (callback_entry_t *),			// sunqueen modify
				 p_var->p_entries,
                 p_var->i_entries,
                 p_var->i_entries,
                 entry );
                     ...
                     vlc_callback_t pf_callback, void *p_data )
{
    ...
    REMOVE_ELEM( (callback_entry_t *), p_var->p_entries, p_var->i_entries, i_entry );			// sunqueen modify
