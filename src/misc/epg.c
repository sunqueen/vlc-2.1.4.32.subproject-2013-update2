 ...
 * $Id: c0148b57750d990d75ee33e2e85ae9d4ca4a8d1b $
 *
                       ...
                       const char *psz_description, uint8_t i_rating )
{
    ...
    TAB_APPEND( (vlc_epg_event_t **), p_epg->i_event, p_epg->pp_event, p_evt );			// sunqueen modify
}
...
void vlc_epg_Merge( vlc_epg_t *p_dst, const vlc_epg_t *p_src )
{
    ...
    for( i = 0; i < p_src->i_event; i++ )
    {
        ...
        if( b_add )
        {
            ...
            TAB_INSERT( (vlc_epg_event_t **), p_dst->i_event, p_dst->pp_event, p_copy, j );				// sunqueen modify
        }
    }
    /* Update current */
    if( p_src->p_current )
