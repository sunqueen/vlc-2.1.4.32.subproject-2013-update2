                                   ...
                                   const char *psz_name )
{
    ...
    PL_LOCK;
    TAB_APPEND( (vlc_sd_internal_t **), pl_priv(p_playlist)->i_sds, pl_priv(p_playlist)->pp_sds, p_sds );			// sunqueen modify
    PL_UNLOCK;
                                      ...
                                      const char *psz_name )
{
    ...
    for( int i = 0; i < priv->i_sds; i++ )
    {
        ...
        {
            p_sds = priv->pp_sds[i];
            REMOVE_ELEM( (vlc_sd_internal_t **), priv->pp_sds, priv->i_sds, i );			// sunqueen modify
            break;
        }
    }
    PL_UNLOCK;
