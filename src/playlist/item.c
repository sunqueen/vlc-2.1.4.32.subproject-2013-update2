...
int playlist_ItemRelease( playlist_item_t *p_item )
{
    ...
    ARRAY_APPEND( (playlist_item_t **), pl_priv(p_playlist)->items_to_delete, p_item);			// sunqueen modify
    return VLC_SUCCESS;
}
                       ...
                       playlist_item_t *p_node, int i_newpos )
{
    ...
    REMOVE_ELEM( (playlist_item_t **), p_detach->pp_children, p_detach->i_children, i_index );			// sunqueen modify

    if( p_detach == p_node && i_index < i_newpos )
        i_newpos--;

    INSERT_ELEM( (playlist_item_t **), p_node->pp_children, p_node->i_children, i_newpos, p_item );			// sunqueen modify
                            ...
                            playlist_item_t *p_node, int i_newpos )
{
    ...
    for( i = 0; i < i_items; i++ )
    {
        ...
        int i_index = ItemIndex( p_item );
        playlist_item_t *p_parent = p_item->p_parent;
        REMOVE_ELEM( (playlist_item_t **), p_parent->pp_children, p_parent->i_children, i_index );			// sunqueen modify
...
static void ChangeToNode( playlist_t *p_playlist, playlist_item_t *p_item )
{
    ...
    if( i != -1 )
        ARRAY_REMOVE( (playlist_item_t **), p_playlist->items, i );			// sunqueen modify
}
