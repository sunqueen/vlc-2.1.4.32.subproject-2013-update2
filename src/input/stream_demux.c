...
stream_t *stream_DemuxNew( demux_t *p_demux, const char *psz_demux, es_out_t *out )
{
    ...
    s->p_sys = p_sys = (stream_sys_t *)malloc( sizeof( *p_sys) );			// sunqueen modify
...
{
    stream_sys_t *p_sys = s->p_sys;
    uint8_t *p_out = (uint8_t *)p_read;			// sunqueen modify
    int i_out = 0;
...
static int DStreamControl( stream_t *s, int i_query, va_list args )
{
    ...
    switch( i_query )
    {
        case STREAM_GET_SIZE:
            p_i64 = (uint64_t *)va_arg( args, uint64_t * );			// sunqueen modify
            *p_i64 = 0;
            return VLC_SUCCESS;
        ...
        {
            uint64_t i64 = (uint64_t)va_arg( args, uint64_t );			// sunqueen modify
            if( i64 < p_sys->i_pos )
                return VLC_EGENERIC;
