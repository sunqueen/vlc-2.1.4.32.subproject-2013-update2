...
#define MISSING(cmd) do { if( !psz_value ) ERROR( "missing argument for " cmd ); } while(0)
    for( i = 0; i < i_property; i++ )
    {
        ...
        {
            MISSING( "input" );
            TAB_APPEND( (char **), p_cfg->i_input, p_cfg->ppsz_input, strdup(psz_value) );			// sunqueen modify
            i++;
        }
        ...
        else if( !strcmp( psz_option, "option" ) )
        {
            ...
            TAB_APPEND( (char **), p_cfg->i_option, p_cfg->ppsz_option, strdup( psz_value ) );			// sunqueen modify
            i++;
        }
...
static vlm_schedule_sys_t *vlm_ScheduleNew( vlm_t *vlm, const char *psz_name )
{
    ...
    TAB_APPEND( (vlm_schedule_sys_t **), vlm->i_schedule, vlm->schedule, p_sched );			// sunqueen modify
                       ...
                       const char *psz_value )
{
    ...
    else if( !strcmp( psz_cmd, "append" ) )
    {
        ...
        TAB_APPEND( (char **), schedule->i_command, schedule->command, command );			// sunqueen modify
    }
    else
    {
        return 1;
                               ...
                               vlm_message_t *p_child )
{
    ...
    if( p_child )
    {
        TAB_APPEND( (vlm_message_t **), p_message->i_child, p_message->child, p_child );			// sunqueen modify
    }
