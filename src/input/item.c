 ...
 * $Id: e2256572cf26a598cc7d41188c817413a1d3d531 $
 *
...
void input_item_Release( input_item_t *p_item )
{
    ...
    if( atomic_fetch_sub(&owner->refs, (atomic_uint)1) != 1 )			// sunqueen modify
        return;
                          ...
                          unsigned flags )
{
    ...
    uint8_t *flagv = (uint8_t *)realloc (p_input->optflagv, p_input->optflagc + 1);			// sunqueen modify
    if (flagv == NULL)
    {
        err = VLC_ENOMEM;
        goto out;
    ...
    INSERT_ELEM( (char **), p_input->ppsz_options, p_input->i_options,			// sunqueen modify
                               ...
                               const char *psz_format, va_list args )
{
    ...
    if( !p_cat )
    {
        ...
        if( !p_cat )
            return VLC_ENOMEM;
        INSERT_ELEM( (info_category_t **), p_i->pp_categories, p_i->i_categories, p_i->i_categories,				// sunqueen modify
                     p_cat );
    }
                        ...
                        const char *psz_name )
{
    ...
    else
    {
        ...
        REMOVE_ELEM( (info_category_t **), p_i->pp_categories, p_i->i_categories, i_cat );				// sunqueen modify
    }
    vlc_mutex_unlock( &p_i->lock );
...
void input_item_ReplaceInfos( input_item_t *p_item, info_category_t *p_cat )
{
    ...
    {
        INSERT_ELEM( (info_category_t **), p_item->pp_categories, p_item->i_categories, p_item->i_categories,				// sunqueen modify
                     p_cat );
    }
    vlc_mutex_unlock( &p_item->lock );
...
void input_item_SetEpg( input_item_t *p_item, const vlc_epg_t *p_update )
{
    ...
    if( !p_epg )
    {
        ...
        if( p_epg )
            TAB_APPEND( (vlc_epg_t **), p_item->i_epg, p_item->pp_epg, p_epg );				// sunqueen modify
    }
    if( p_epg )
...
signal:
    ...
    if( p_epg->i_event > 0 )
    {
        vlc_event_t event = { /*.type = */vlc_InputItemInfoChanged, };				// sunqueen modify
        vlc_event_send( &p_item->event_manager, &event );
    }
}
...
//    char *ppsz_epg_info[i_epg_info];
	char **ppsz_epg_info = (char **)malloc(sizeof(char *) * i_epg_info);

	memset(ppsz_epg_info, '\0', sizeof(char *) * i_epg_info);
	// sunqueen modify end
	...
	free(ppsz_epg_info);			// sunqueen add
}
                        ...
                        unsigned flags, mtime_t duration, int type )
{
    ...
    input_item_owner_t *owner = (input_item_owner_t *)calloc( 1, sizeof( *owner ) );				// sunqueen modify
    ...
    p_input->i_id = atomic_fetch_add(&last_input_id, (atomic_uint)1);			// sunqueen modify
...
{
    const struct item_type_entry *type = (const item_type_entry *)entry;			// sunqueen modify
    const char *uri = (const char *)key, *scheme = (const char *)type->psz_scheme;			// sunqueen modify
...
static int GuessType( const input_item_t *p_item )
{
    ...
    e = (const item_type_entry *)bsearch( p_item->psz_uri, tab, sizeof( tab ) / sizeof( tab[0] ),				// sunqueen modify
...
{
    input_item_node_t* p_node = (input_item_node_t *)malloc( sizeof( input_item_node_t ) );				// sunqueen modify
    if( !p_node )
        return NULL;
...
void input_item_node_Delete( input_item_node_t *p_node )
{
    if( p_node->p_parent )
        for( int i = 0; i < p_node->p_parent->i_children; i++ )
            ...
            {
                REMOVE_ELEM( (input_item_node_t **), p_node->p_parent->pp_children,				// sunqueen modify
...
void input_item_node_AppendNode( input_item_node_t *p_parent, input_item_node_t *p_child )
{
    ...
    INSERT_ELEM( (input_item_node_t **),				// sunqueen modify
				 p_parent->pp_children,
                 p_parent->i_children,
                 p_parent->i_children,
                 p_child );
...
{
    int i;
    es_format_t *fmt_copy = (es_format_t *)malloc(sizeof *fmt_copy);			// sunqueen modify
    if (!fmt_copy)
        return;
    ...
    TAB_APPEND( (es_format_t **), item->i_es, item->es, fmt_copy);			// sunqueen modify
    vlc_mutex_unlock( &item->lock );
}
