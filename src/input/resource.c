                       ...
                       size_t *pi_vout )
{
    ...
    *ppp_vout = NULL;
    ...
    pp_vout = (vout_thread_t **)malloc( p_resource->i_vout * sizeof(*pp_vout) );			// sunqueen modify
    if( !pp_vout )
        goto exit;
...
{
    input_resource_t *p_resource = (input_resource_t *)calloc( 1, sizeof(*p_resource) );			// sunqueen modify
    if( !p_resource )
        return NULL;
...
{
    if( atomic_fetch_sub( &p_resource->refs, (atomic_uint)1 ) != 1 )			// sunqueen modify
        return;
...
{
    atomic_fetch_add( &p_resource->refs, (atomic_uint)1 );			// sunqueen modify
    return p_resource;
}
