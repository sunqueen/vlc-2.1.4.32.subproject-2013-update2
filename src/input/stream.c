...
//    char psz_dup[strlen( psz_url ) + 1];
	char *psz_dup = (char *)malloc(strlen( psz_url ) + 1);
	memset(psz_dup, '\0', strlen( psz_url ) + 1);
	// sunqueen modify end
    ...
    if( p_access == NULL )
    {
		...
		free(psz_dup);			// sunqueen add
        return NULL;
    }

    /*return*/ p_res = stream_AccessNew( p_access, NULL );			// sunqueen modify
	free(psz_dup);			// sunqueen add
	return p_res;			// sunqueen add
}
...
stream_t *stream_AccessNew( access_t *p_access, char **ppsz_list )
{
    ...
    if( ppsz_list && ppsz_list[0] )
    {
        ...
        TAB_APPEND( (access_entry_t **), p_sys->i_list, p_sys->list, p_entry );			// sunqueen modify
