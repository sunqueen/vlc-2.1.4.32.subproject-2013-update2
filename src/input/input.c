...
static void LoadSubtitles( input_thread_t *p_input )
{
    ...
    for( int i = 0; i < p_input->p->i_attachment; i++ )
    {
        ...
        if( !strcmp( a->psz_mime, "application/x-srt" ) )
            TAB_APPEND( (input_attachment_t **), i_attachment, pp_attachment,			// sunqueen modify
...
static void LoadSlaves( input_thread_t *p_input )
{
    ...
    while( psz && *psz )
    {
        ...
        if( p_slave && !InputSourceInit( p_input, p_slave, uri, NULL, false ) )
            TAB_APPEND( (input_source_t **), p_input->p->i_slave, p_input->p->slave, p_slave );			// sunqueen modify
        else
            free( p_slave );
        free( uri );
    }
    ...
    if( p_input->p->p_sout )
    {
        ...
        if( (prgms = var_GetNonEmptyString( p_input, "programs" )) != NULL )
        {
                 ...
                 prgm = strtok_r( NULL, ",", &buf ) )
            {
                ...
                INSERT_ELEM( (vlc_value_t *), list.p_values, list.i_count, list.i_count, val );			// sunqueen modify
            }
        ...
        case INPUT_CONTROL_ADD_SLAVE:
            if( val.psz_string )
            {
                ...
                if( slave && !InputSourceInit( p_input, slave, uri, NULL, false ) )
                {
                    ...
                    TAB_APPEND( (input_source_t **), p_input->p->i_slave, p_input->p->slave, slave );			// sunqueen modify
                }
                else
                {
                    free( slave );
                            ...
                            const char *psz_forced_demux, bool b_in_can_fail )
{
    ...
    {   /* Now try a real access */
        ...
        if( &p_input->p->input == in )
        {
            ...
            while( psz_parser && *psz_parser )
            {
                ...
                if( *psz_parser )
                {
                    ...
                    if( psz_name )
                        TAB_APPEND( (char **), i_input_list, ppsz_input_list, psz_name );			// sunqueen modify
                }
        ...
        if( i_input_list > 0 )
            TAB_APPEND( (char **), i_input_list, ppsz_input_list, NULL );			// sunqueen modify
...
static void InputMetaUser( input_thread_t *p_input, vlc_meta_t *p_meta )
{
    static const struct { int i_meta; /*const*/ char *psz_name; } p_list[] = {			// sunqueen modify
                                       ...
                                       int i_start, int i_stop )
{
    ...
    *psz_end = '\0';
    ...
    for( int i = i_start; i <= i_stop; i++ )
    {
        ...
        TAB_APPEND( (char **), i_list, ppsz_list, psz_file );			// sunqueen modify
        free( psz_tmp_path );
    }
    free( psz_base );
exit:
                               ...
                               const char *url, unsigned i_flags )
{
    ...
    TAB_APPEND( (input_source_t **), p_input->p->i_slave, p_input->p->slave, sub );			// sunqueen modify
