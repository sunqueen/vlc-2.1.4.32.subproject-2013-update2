...
static es_out_pgrm_t *EsOutProgramAdd( es_out_t *out, int i_group )
{
    ...
    /* Append it */
    TAB_APPEND( (es_out_pgrm_t **), p_sys->i_pgrm, p_sys->pgrm, p_pgrm );			// sunqueen modify
...
static es_out_id_t *EsOutAdd( es_out_t *out, const es_format_t *fmt )
{
    ...
    TAB_APPEND( (es_out_id_t **), out->p_sys->i_es, out->p_sys->es, es );			// sunqueen modify
...
static char **LanguageSplit( const char *psz_langs, bool b_default_any )
{
    ...
    while( psz_parser && *psz_parser )
    {
        ...
        {
            TAB_APPEND( (char **), i_psz, ppsz, strdup("any") );			// sunqueen modify
        }
        else if( !strcmp( psz_parser, "none" ) )
        {
            TAB_APPEND( (char **), i_psz, ppsz, strdup("none") );			// sunqueen modify
        }
        else
        {
            ...
            if( strcmp( psz_code, "??" ) )
            {
                TAB_APPEND( (char **), i_psz, ppsz, psz_code );			// sunqueen modify
            }
            else
            {
                free( psz_code );
    ...
    if( i_psz )
    {
        if( b_default_any && strcmp( ppsz[i_psz - 1], "none" ) )
            TAB_APPEND( (char **), i_psz, ppsz, strdup("any") );			// sunqueen modify
        TAB_APPEND( (char **), i_psz, ppsz, NULL );			// sunqueen modify
    }
