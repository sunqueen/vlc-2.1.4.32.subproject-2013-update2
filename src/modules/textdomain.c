...
//    if (lpath == NULL || bindtextdomain (domain, lpath) == NULL)
    if (lpath == NULL || cover_bindtextdomain (domain, lpath) == NULL)			// sunqueen modify
    {
        free (lpath);
        fprintf (stderr, "%s: text domain not found in %s\n", domain, upath);
        free (upath);
...
//    if (bind_textdomain_codeset (PACKAGE_NAME, "UTF-8") == NULL)
    if (cover_bind_textdomain_codeset (PACKAGE_NAME, "UTF-8") == NULL)			// sunqueen modify
    {
...
//        bindtextdomain (PACKAGE_NAME, "/DOES_NOT_EXIST");
        cover_bindtextdomain (PACKAGE_NAME, "/DOES_NOT_EXIST");			// sunqueen modify
        return -1;
    }
...
#ifdef ENABLE_NLS
    if (likely(*msgid))
        return cover_dngettext (PACKAGE_NAME, msgid, plural, n);			// sunqueen modify
#endif
